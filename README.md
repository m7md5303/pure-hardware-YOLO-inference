...
<p> This notebook is almost the same as the previous one. However, it reduces the paralleslism in the network (folding) in order to decrease the needed resources and be able to deploy it on PYNQ. The goal from this is visualizing the final output through the PYNQ Jupyter Notebook.</p>
...
<h3> <a href='https://github.com/m7md5303/pure-hardware-YOLO-inference/blob/main/Y_Verification.ipynb'> 5-Post-Processing Block</a> </h3>
<p> This Verilog module is the main core of the hardware inference. It post-processes the RAW output from the YOLO network without any interaction from the software. The used data was only related to the class score and objectness score where there was no care about the position of the object inside its grid but it was sufficient to know whether a grid has an object or no.</p>

<h3> <a href='https://github.com/m7md5303/pure-hardware-YOLO-inference/blob/main/Sys%20bd.png'> 6-System Block Design </a> </h3>
<p> This how the system should look like for simulation. It is also possible if you want to integrate it in your project or use it only in a hardware project. Just take care of the IO mapping to generate your bitstream properly</p>
